                    ...
                    <div class="col-md-5 col-sm-5   form-group pull-right top_search">
                        <nav aria-label="breadcrumb" >
                            <ol class="breadcrumb pull-right" style="background: none">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">View/Edit Halls</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
<!--                                <h2>Default Example <small>Users</small></h2>-->
                            <a href="{{ url('/view_edit_hall/0') }}" class="btn btn-success btn-sm pull-right text-white mt-3 mb-3"><i class="fa fa-plus"></i> &nbsp; Add New Hall Type&nbsp; </a>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">
                            {{ var_dump($addroomtypes_status) }}
                            @if(strtolower($role)=='super'||strtolower($role)=='admin')
                            <form action="{{ url('/view_edit_hall/'.$roomtype_id) }}" method="POST" enctype="multipart/form-data" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left mt-5">
                                @csrf
                                    ...
                                    <div class="item form-group">
                                        <div class="col-md-6 col-sm-6 offset-md-3">
                                            <button type="submit" class="btn btn-primary" type="button" name="add_hall" value="cancel_add_hall">Cancel</button>
                                            <button type="submit" class="btn btn-success" name="add_hall" value="@if($roomtype_id>0) {{ "update_hall" }}@else  {{ "add_hall" }} @endif">@if($roomtype_id>0) {{ "Update Hall..." }}@else  {{ "Add New Hall..." }} @endif</button>
                                        </div>
                                    </div>
